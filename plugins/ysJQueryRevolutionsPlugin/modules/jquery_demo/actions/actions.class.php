...
class jquery_demoActions extends sfActions
{
  ...
  public function executeAutocompleteData(sfWebRequest $request){
    ...
    $response = '';
    $items = $this->getData($dataTypeParam);
  ...
  public function executeAutocompleteImageData(sfWebRequest $request){
    $this->forward404If(!$request->isXmlHttpRequest());
    $q = $request->getParameter('q');
    $dataTypeParam = $request->getParameter('dataTypeParam');
    $response = '';
    ...
    foreach($images as $image){
      $image = trim($image);
      if( strpos(strtolower($image), $q) === 0 ) {
        $response .= $image . "\n";
      }
    }
    return $this->renderText($response);
  ...
  private function getData($type = 'images'){
    $data = array();
    switch ($type) {
      case 'images':
        $data =     array('Amsterdam.jpg' , '
        ...
        break;
      case 'items':
            $data = array("Great <em>Bittern</em>"=>"Botaurus stellaris",
